...
public class TopicRequestService : BaseService<TopicRequest>, ITopicRequestService
{
    ...
    public async Task<BusinessResult> CreateCouncilRequestsForTopic(TopicRequestCreateForCouncilsCommand command)
    {
        try
        {
            ...
            if (newTopicVersionRequests.Count == 0) return HandlerNotFound("Không có người dùng có thể tham gia hội đồng");
        ...
        TopicRequestLecturerOrCouncilResponseCommand command)
    {
        try
        {
            var topicRequest = await _topicRequestRepository.GetById(command.Id, true);
